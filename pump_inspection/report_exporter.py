...
<body>
    <div class="container">
        ...
        {% if batch_remarks or anomaly_remarks %}
        <h2>备注信息</h2>
        {% if batch_remarks %}
        <h3>批次级备注</h3>
        <table>
        ...
        <table>
            ...
            <tbody>
                {% for remark in anomaly_remarks %}
                <tr>
                    <td>#{{ remark.id }}</td>
                    <td>#{{ remark.anomaly_id }}</td>
                    <td>{{ remark.remark_type }}</td>
                    <td>{{ remark.operator }}</td>
                    <td>{{ remark.created_at }}</td>
                    <td>{{ remark.content }}</td>
...
class ReportExporter:
    ...
    def export_html(self, batch_id: int, include_raw_data: bool = True, filename: Optional[str] = None) -> str:
        ...
        batch_remarks = self.db.query(Remark).filter(
            Remark.batch_id == batch_id,
            Remark.anomaly_id.is_(None)
        ).order_by(Remark.created_at.desc()).all()

        anomaly_remarks = self.db.query(Remark).filter(
            Remark.batch_id == batch_id,
            Remark.anomaly_id.isnot(None)
        ).order_by(Remark.created_at.desc()).all()

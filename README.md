...
- **回滚机制**: 支持回滚复核结果
- **备注管理**: 支持批次级和异常级备注，可从 CSV 批量导入或单条追加，支持去重
- **规则版本管理**: 支持多版本检测规则，可切换、可对比
- **报告导出**: 支持 HTML 和 CSV 格式报告导出，包含备注信息
...
```
**重复导入行为说明**：
- **不同批次**：相同班次编号可正常导入，每个批次独立维护自己的班次记录
- **同一批次重复导入**：执行 upsert（存在则更新，不存在则新增），不会报错
- 输出示例：`巡检班次导入完成: 总计 5 条 (新增 5, 更新 0, 跳过 0)`
- 重复导入同一文件时：`巡检班次导入完成: 总计 5 条 (新增 0, 更新 5, 跳过 0)`
